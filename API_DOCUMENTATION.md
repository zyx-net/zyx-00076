...
**重要规则**:
- 只能对 `active` 状态的时限进行催办
- 同一 `active` 时限只能有一次**未消化**的手动催办
- "未消化"定义：最后一次手动催办之后，没有发生以下任何"消化"动作：
  - 暂停时限 (`paused`)
  - 恢复时限 (`resumed`)
  - 完成时限 (`completed`)
  - 关闭时限 (`closed`)
  - 重新计算时限 (`recalculated`)
- 自动催办（首次/二次/升级）不属于消化动作，不会清除未消化的手动催办状态
- 重复催办会返回 400 错误，且不会新增审计日志
...
### SLA配置管理
1. **优先级匹配**：按priority从高到低匹配，数值越大优先级越高
2. **最具体规则优先**：指定了最多匹配条件的规则优先于通用规则
3. **配置校验**：创建/更新时强制校验催办时间<时限<升级时间的逻辑
4. **激活状态控制**：停用的SLA配置不会被匹配，但已创建的时限不受影响

### 时限生命周期管理
1. **单步单一活跃时限**：每个审批步骤同一时间只能有一个活跃时限
2. **状态机严格转换**：active↔paused→completed→closed，状态转换有严格校验
3. **操作留痕**：所有状态变更（创建、暂停、恢复、催办、完成、关闭）都写入审计日志
4. **避免重复催办**：自动催办使用sent标记，同一级别的催办只会发送一次
5. **流程钩子集成**：在审批提交、补件、驳回、归档、步骤完成等7个钩子点自动管理时限
6. **异常场景处理**：合同状态变更但时限未正常关闭时，归档操作会强制关闭所有时限

### 自动催办与升级
1. **定时任务**：每分钟检查一次是否有需要催办或升级的时限
2. **暂停保护**：暂停状态的时限不会触发自动催办和升级
3. **完成保护**：已完成的时限不会继续触发催办和升级
4. **手动优先**：管理员可随时手动催办，不受自动催办频率限制

### 审计日志设计
1. **完整信息记录**：每条日志包含操作者、原因、前后状态、IP地址和时间戳
2. **旧值新值保存**：重新计算等操作会保存变更前后的具体值
3. **查询权限控制**：普通用户只能查看自己相关时限的审计日志
4. **永久留存**：审计日志不会被删除，用于追溯和审计
